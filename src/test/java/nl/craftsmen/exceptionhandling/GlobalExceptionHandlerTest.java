...
import static nl.craftsmen.contacts.ContactsTestdataSupplier.*;
...
class GlobalExceptionHandlerTest {
    ...
    private static final String RESOURCE_NOT_FOUND_ERROR_MESSAGE = "Resource with id " + ID + " not found";
    ...
    void handleValidationExceptionMethodArgumentNotValidExceptionExpectResponse400AndApiErrorWithErrorMessageAndValidatieMessage() {
        ...
        assertThat(Objects.requireNonNull(apiError)
                .getErrorMessage()).isEqualTo(METHOD_ARGUMENT_NOT_VALID_ERROR_MESSAGE);
    ...
    void handleExceptionMethodArgumentTypeMismatchExceptionExpectResponse400AndApiErrorWithErrorMessageButNoValidationMessages() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(Objects.requireNonNull(apiError)
                .getErrorMessage()).isEqualTo(METHOD_ARGUMENT_TYPE_MISMATCH_ERROR_MESSAGE);
    ...
    @Test
    @DisplayName("When a ResourceNotFoundException occurs, I expect a Not Found response with an ApiError and error "
            + "message, but no validation messages")
    void handleExceptionResourceNotFoundExceptionExpectResponse404AndApiErrorWithErrorMessageAndNoValidationMessages() {
        // GIVEN
        final var resourceNotFoundException = new ResourceNotFoundException(ID);

        // WHEN
        final var response = cut.handelException(resourceNotFoundException);

        // THEN
        final var apiError = response.getBody();
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(Objects.requireNonNull(apiError).getErrorMessage()).isEqualTo(RESOURCE_NOT_FOUND_ERROR_MESSAGE);
        assertThat(apiError.getValidationMessages()).isEmpty();
    }
